...
repdem_pot_coal_paco <- read_datasets("repdem_pot_coal_pastr", db, original = TRUE)

# Clean column names
names(repdem_pot_coal_paco) <- clean_column_names(names(repdem_pot_coal_paco))

# Duplicates check to identify units
no_duplicates(repdem_pot_coal_paco, c("cab_id", "potential_coalition")) #TRUE

# Create unit columns and add a country_name column
repdem_pot_coal_paco$u_repdem_cabinet_pot_coal_cab_id <- 
  repdem_pot_coal_paco$cab_id

repdem_pot_coal_paco$u_repdem_cabinet_pot_coal_coalition <- 
  repdem_pot_coal_paco$potential_coalition

repdem_pot_coal_paco %<>%
...
no_duplicate_names(repdem_pot_coal_paco)

write_dataset(repdem_pot_coal_paco, 
              file.path(Sys.getenv("ROOT_DIR"),
                        "datasets/repdem/cleaned_datasets/repdem_pot_coal_paco_cleaned.rds"),
              tag = "repdem_pot_coal_paco",
              overwrite = TRUE)

# Create static files in dta and csv format
write_file(repdem_pot_coal_paco,
           file.path(Sys.getenv("ROOT_DIR"),
                     "datasets/repdem/cleaned_datasets_dta/repdem_pot_coal_paco_cleaned.dta"),
           overwrite = TRUE)

write_file(repdem_pot_coal_paco,
           file.path(Sys.getenv("ROOT_DIR"),
                     "datasets/repdem/cleaned_datasets_csv/repdem_pot_coal_paco_cleaned.csv"),
           overwrite = TRUE)