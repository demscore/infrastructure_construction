...
repdem_paco_month <- read_datasets("repdem_paco_month", db, original = TRUE)

# Clean column names
names(repdem_paco_month) <- clean_column_names(names(repdem_paco_month))

# Duplicates check to identify units
no_duplicates(repdem_paco_month, c("cab_id", "month")) #TRUE

# Create unit columns and add a country_name column
repdem_paco_month$u_repdem_cabinet_month_cab_name <- 
  repdem_paco_month$cab_name

repdem_paco_month$u_repdem_cabinet_month_month <- 
  format(as.Date(repdem_paco_month$month, format="%Y-%m-%d"),"%m-%Y")
...
no_duplicate_names(repdem_paco_month)

write_dataset(repdem_paco_month, 
              file.path(Sys.getenv("ROOT_DIR"),
                        "datasets/repdem/cleaned_datasets/repdem_paco_month_cleaned.rds"),
              tag = "repdem_paco_month",
              overwrite = TRUE)

# Create static files in dta and csv format
write_file(repdem_paco_month,
           file.path(Sys.getenv("ROOT_DIR"),
                     "datasets/repdem/cleaned_datasets_dta/repdem_paco_month_cleaned.dta"),
           overwrite = TRUE)

write_file(repdem_paco_month,
           file.path(Sys.getenv("ROOT_DIR"),
                     "datasets/repdem/cleaned_datasets_csv/repdem_paco_month_cleaned.csv"),
           overwrite = TRUE)