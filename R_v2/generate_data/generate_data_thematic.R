...
args <- list(help = FALSE,
			 ...
			 outfile = file.path(OUT_DIR, paste0(THEME, "_dataset_policy.rds")),
...
df <- read_file(file.path(OUT_DIR, paste0(THEME, "_dataset_policy.rds")))

write_file(df, file.path(OUT_DIR, paste0(THEME, "_dataset_policy.csv")))

write_file(df, file.path(OUT_DIR, paste0(THEME, "_dataset_policy.dta")))
...
args$thematic <- THEME
args$outfile <- file.path(OUT_DIR, paste0(THEME, "_codebook_policy.pdf"))
