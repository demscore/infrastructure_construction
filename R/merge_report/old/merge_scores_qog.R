...
df <- read_datasets("qog_std_ts", db)
qog_org_scores <- calculate_scores_org_data(df, "qog_std_ts_")
...
df <- read_unit_data("u_qog_country_year", "qog_std_ts",  variables, datasets)
...
qog_end_scores <- calculate_merge_score_end_unit(test)

### -----------------------------------------------------------
### Calculate the absolute and relative loss for each variable
### -----------------------------------------------------------

loss <- left_join(qog_org_scores, qog_end_scores, by = c("variable")) %>%
