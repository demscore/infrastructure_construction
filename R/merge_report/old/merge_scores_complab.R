...
df <- read_datasets("complab_spin_cbd", db)
complab_org_scores <- calculate_scores_org_data(df, "complab_spin_cbd_")
...
df <- read_unit_data("u_complab_country_year", "complab_spin_cbd",  variables, datasets)

test <- to_u_vdem_country_year(df)

### -----------------------------------------------------------
### Define function calculating absolute and percentage scores
### based on non missing observations for every Complab SPIN 
### Samip variable in the V-Dem CY Output Unit
### -----------------------------------------------------------
...
complab_end_scores <- calculate_merge_score_end_unit(test)

### -----------------------------------------------------------
### Calculate the absolute and relative loss for each variable
### -----------------------------------------------------------

loss <- left_join(complab_org_scores, complab_end_scores, by = c("variable")) %>%
