...
year_coverage_hdata <- function(p) {
  ...
  df %<>% select(tag, name, year_coverage, demscore_release) %>%
    ...
    plot <- ggplot(df, aes(y = reorder(name, min_year), x = min_year, xend = max_year)) +
      ...
      scale_x_continuous(breaks = seq(1740, 2025, 10),
                     limits=c(1740, 2025)) 

      return(plot)
}

plot <- year_coverage_hdata("hdata")
plot

ggsave(file.path(Sys.getenv("ROOT_DIR"), "figures/ds_year_coverage/hdata.jpg"), width = 14, height = 4)
