...
#complab_spin_outwb_old <- read_datasets("complab_spin_outwb", db, original = TRUE)

complab_spin_outwb <- read_datasets("complab_spin_outwb", db, original = TRUE)

# Clean column names
names(complab_spin_outwb) <- clean_column_names(names(complab_spin_outwb))
...
complab_spin_outwb %<>% rename(country_code = country)
complab_spin_outwb %<>% rename(country_nr = countrynr) 

# Duplicates check to identify units
no_duplicates(complab_spin_outwb, c("country_nr", "year")) #TRUE
no_duplicates(complab_spin_outwb, c("country_code", "year")) #TRUE


# Create unit columns
#country_nr commented out bc migpol datasets do not have a complab_country_nr
#complab_spin_outwb$u_complab_country_year_country_nr <- 
#  as.numeric(complab_spin_outwb$country_nr)

complab_spin_outwb$u_complab_country_year_country_code <- 
  complab_spin_outwb$country_code

complab_spin_outwb$u_complab_country_year_year <- 
  as.integer(complab_spin_outwb$year)
...
complab_spin_outwb %<>% 
  mutate(u_complab_country_year_country = case_when(
    ...
    u_complab_country_year_country_code == "HRV" ~ "Croatia", 
    u_complab_country_year_country_code == "RUS" ~ "Russia",
...
no_duplicate_names(complab_spin_outwb)


write_dataset(complab_spin_outwb, 
           file.path(Sys.getenv("ROOT_DIR"),
                     "datasets/complab/cleaned_datasets/complab_spin_outwb_cleaned.rds"),
           tag = "complab_spin_outwb",
           overwrite = TRUE)

# Create static files in dta and csv format
write_file(complab_spin_outwb,
           file.path(Sys.getenv("ROOT_DIR"),
                     "datasets/complab/cleaned_datasets_dta/complab_spin_outwb_cleaned.dta"),
           overwrite = TRUE)

write_file(complab_spin_outwb,
           file.path(Sys.getenv("ROOT_DIR"),
                     "datasets/complab/cleaned_datasets_csv/complab_spin_outwb_cleaned.csv"),
           overwrite = TRUE)