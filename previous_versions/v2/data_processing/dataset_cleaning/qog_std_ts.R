...
# Remove country-year observations for countries before their independence
clean_years <- read_file(file.path(Sys.getenv("ROOT_DIR"), "datasets", "qog", "clean_years_qog_std_ts.rds"))

# WIP, this leaves 202 as max year and would remove 2021 and 2022 from qog_std
qog_std_ts <- left_join(clean_years, qog_std_ts, by = c("ccode_qog", "year")) %>%
  filter(!(is.na(cname_qog)))
...
write_file(qog_std_ts,
           file.path(Sys.getenv("ROOT_DIR"),
                     "datasets/qog/cleaned_datasets_dta/qog_std_ts_cleaned.dta"),
           overwrite = TRUE)

write_file(qog_std_ts,
           file.path(Sys.getenv("ROOT_DIR"),
                     "datasets/qog/cleaned_datasets_csv/qog_std_ts_cleaned.csv"),
           overwrite = TRUE)