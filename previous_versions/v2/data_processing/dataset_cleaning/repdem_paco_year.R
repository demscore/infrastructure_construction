...
repdem_paco_year <- read_datasets("repdem_paco_year", db, original = TRUE)

# Clean column names
names(repdem_paco_year) <- clean_column_names(names(repdem_paco_year))

# Duplicates check to identify units
no_duplicates(repdem_paco_year, c("cab_id", "year")) #TRUE

# Create unit columns and add a country_name column
repdem_paco_year$u_repdem_cabinet_year_cab_name <- 
  repdem_paco_year$cab_name

repdem_paco_year$u_repdem_cabinet_year_year <- 
  format(as.Date(repdem_paco_year$year, format="%Y-%m-%d"),"%Y")
...
no_duplicate_names(repdem_paco_year)

write_dataset(repdem_paco_year, 
              file.path(Sys.getenv("ROOT_DIR"),
                        "datasets/repdem/cleaned_datasets/repdem_paco_year_cleaned.rds"),
              tag = "repdem_paco_year",
              overwrite = TRUE)

# Create static files in dta and csv format
write_file(repdem_paco_year,
           file.path(Sys.getenv("ROOT_DIR"),
                     "datasets/repdem/cleaned_datasets_dta/repdem_paco_year_cleaned.dta"),
           overwrite = TRUE)

write_file(repdem_paco_year,
           file.path(Sys.getenv("ROOT_DIR"),
                     "datasets/repdem/cleaned_datasets_csv/repdem_paco_year_cleaned.csv"),
           overwrite = TRUE)