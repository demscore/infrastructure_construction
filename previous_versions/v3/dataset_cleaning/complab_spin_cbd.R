...
complab_spin_cbd <- read_datasets("complab_spin_cbd", db, original = TRUE)

# Clean column names
names(complab_spin_cbd) <- clean_column_names(names(complab_spin_cbd))
...
complab_spin_cbd %<>% rename(country_code = country)
complab_spin_cbd %<>% rename(country_nr = countrynr) 

# Duplicates check to identify units
no_duplicates(complab_spin_cbd, c("country_nr", "year")) #TRUE
no_duplicates(complab_spin_cbd, c("country_code", "year")) #TRUE


# Add unit columns
#complab_spin_cbd$u_complab_country_year_country_nr <- 
#  as.numeric(complab_spin_cbd$country_nr)

complab_spin_cbd$u_complab_country_year_country_code <- 
  complab_spin_cbd$country_code

complab_spin_cbd$u_complab_country_year_year <- 
  as.integer(complab_spin_cbd$year)
...
no_duplicate_names(complab_spin_cbd)


write_dataset(complab_spin_cbd,
           file.path(Sys.getenv("ROOT_DIR"),
                     "datasets/complab/cleaned_datasets/complab_spin_cbd_cleaned.rds"),
           tag = "complab_spin_cbd",
           overwrite = TRUE)

# Create static files in dta and csv format
write_file(complab_spin_cbd,
           file.path(Sys.getenv("ROOT_DIR"),
                     "datasets/complab/cleaned_datasets_dta/complab_spin_cbd_cleaned.dta"),
           overwrite = TRUE)

write_file(complab_spin_cbd,
           file.path(Sys.getenv("ROOT_DIR"),
                     "datasets/complab/cleaned_datasets_csv/complab_spin_cbd_cleaned.csv"),
           overwrite = TRUE)