...
#check <- read_file(file.path(Sys.getenv("ROOT_DIR"), "themes", "3.0", "migration_dataset_policy.rds"))
#check %<>% filter(u_complab_country_year_policy_country == "Sweden")
...
args <- list(help = FALSE,
             ...
             file_format = "R", 
             output_unit_tag = OUTPUT_UNIT,
             outfile = file.path(OUT_DIR, paste0(THEME, "_dataset_party_countr_year.rds")),
             select_all_rows = TRUE,
             country = c("Norway"),
             min_year = 1900,
             max_year = 2019,
...
df <- read_file(file.path(OUT_DIR, paste0(THEME, "_dataset_party_countr_year.rds")))
write_file(df, file.path(OUT_DIR, paste0(THEME, "_dataset_party_countr_year.csv")))
write_file(df, file.path(OUT_DIR, paste0(THEME, "_dataset_party_countr_year.dta")))
...
args$thematic <- THEME
args$outfile <- file.path(OUT_DIR, paste0(THEME, "_codebook_party_countr_year.pdf"))
create_codebook(args, REF_STATIC_DIR, POSTGRES_TABLES_DIR, 
                LOCAL, prep = FALSE)